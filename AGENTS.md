...
- Autonomous queue loop (the LIVE workflow): the live work queue is `cocalc-shared/codex-queue.md` — read it each turn; it is your only work source. Contract: `cocalc-shared/codex-plan.md` (the loop + the validation gate + the protected-surface gate), which supersedes `codex-kickoff-prompt.md` and `codex-landing-page-guardrails.md`. `cocalc-shared/INDEX.md` routes the rest. The command-library rounds below apply only to non-queue exploratory work.
- **`docs/website-operating-system.md`** — the durable operating model: autonomous queue implementation, Claude's per-commit audit, Blaec's batch/override arbitration, and the slow `/pitch-challenge` outer loop. Read it with `cocalc-shared/codex-plan.md`.
- For explicit non-queue work, use the commands in `src/.claude/commands/`: `/site-round <route>`, `/site-audit`, `/site-verify`, `/site-judge`, `/pitch-challenge`. Codex/Gemini: these are plain prompts — read the matching `*.md`.
