...
SSH access to projects uses sshpiperd and an assembled collection of `authorized_keys` sources, including the PostgreSQL database and the `~/.ssh/authorized_keys` file on disk. By using sshpiperd, we get full support for port forwarding, X11 forwarding, etc., and also can route based on the username.
