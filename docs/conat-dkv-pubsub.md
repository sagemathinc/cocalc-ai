...
// Multibay rule: resolve the project's conat client EXPLICITLY — with the
// implicit home-bay connection, collaborators homed on different bays would
// open the "same" DKV against different fabrics and never see each other.
...
Outside the frontend (backend, CLI), import `dkv` from `@cocalc/conat/sync/dkv`
directly and pass an explicit `client`.
...
class MyActions {
  ...
  async init(project_id: string) {
    // Multibay rule: resolve the project's conat client explicitly.
    const client = await webapp_client.conat_client.projectConat({
      project_id,
      caller: "my-feature",
      requireRouting: true,
    });
    this.store = await webapp_client.conat_client.dkv<MyState>({
      client,
      project_id,
      name: "my-feature",
      ephemeral: true,
...
// PubSub requires an explicit Conat client — the caller owns routing.
// For project-scoped pubsub, resolve the project's client explicitly
// (multibay rule) instead of using the home-bay connection:
const client = await webapp_client.conat_client.projectConat({
  project_id,
  caller: "my-feature",
  requireRouting: true,
...
4. **Synchronous constructor** — subscribes internally (no async init needed),
   but throws immediately if no explicit `client` is passed.
