...
- Storage usage recompute buttons do not imply fresh data unless a real sample
  ran. **Fixed 2026-05-22** with explicit recompute status and backend
  rate-limiting.
- Backup timestamps do not knowingly lie.
...
| Disk usage reload says "Updated just now" without recomputing | storage UI / quota | fixed  | The storage modal now uses an explicit `Recompute` action. Successful recomputes update the overview directly from the returned sample, show the actual measured time, bypass cached `du` scans only when allowed, and report rate-limited or in-flight recompute attempts honestly.                       |
...
- Disk usage reload lie. **Fixed 2026-05-22.**
