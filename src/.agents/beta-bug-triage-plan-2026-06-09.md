...
### (done) P1-E: Docs Pages No Longer Render Markdown Code Nicely

Status: Fixed. Public docs now render through the Slate static markdown renderer, lazy-loaded to keep the initial public bundle under the static package entry budget. Heading ids generated by Slate markdown were also tightened to CSS-safe slugs so docs print/single-page rendering does not trip Ant Design selector handling.
...
- Completed: `cd src/packages/frontend && pnpm test -- public/docs/__tests__/app.test.tsx docs/browser.test.tsx editors/slate/__test__/static-markdown-heading-id.test.tsx`.
- Completed: `cd src/packages/frontend && pnpm tsc --build`.
