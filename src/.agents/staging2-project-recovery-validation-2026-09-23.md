...
1. Snapshot restore with `mode=home` failed on staging2 because the existing
   restore path attempts to rename `.local/share/cocalc/rootfs` from within a
   Btrfs project subvolume to the mount-level staging directory. The two paths
   have different device IDs, so the secure rename helper returned `EXDEV`.
   The failed operation left the project file intact, but the project needed
   an explicit start before project execution responded. `mode=both` passed.
   Fix and retest the home-only path before claiming full snapshot recovery.
