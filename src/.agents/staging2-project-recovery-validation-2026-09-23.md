...
`c90c2e7e-9a1f-46da-bd7f-9d5222fd1c34`. These disposable projects remain on
...
3. The plan's full observability and scheduler contract remains broader than
   the current code: stage-specific timing and bytes, versioned host ledger,
   bounded ownership leases, pressure debt and capacity reports, operator drill
   reporting, and gated automatic rollout are not yet present. Event-triggered
   due work and mutation-boundary assignment checks are now deployed; the full
   inventory still reconciles on a 15-minute timer.
