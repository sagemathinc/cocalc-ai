...
- [ ] Keep date only for sort/time display. (partial: thread grouping + ACP queue/session identity now prefer `thread_id`; thread-config reads are now `thread_id` only; date remains for ordering/UI keys and selected legacy updates)
