...
- Keep auditability, idempotency, and handoff to project agents from day one.
...
- **Full CoCalc hub:** multi-user, multi-project, Postgres-backed control plane.
- **CoCalc+Plus hub:** single-user, single-project, local app-like environment (no collaborators).

## Minimal Vertical Slice (touch each area)

- **Agent runner:** In-hub agent loop with a single control agent using OpenAI Agents SDK.
- **Tool surface:** Adapter-driven tools; full hub gets project/collab tools, lite hub gets config/logs/editor/sync tools.
- **UI actions:** Client-side allowlisted navigation actions (open project, switch to projects list, highlight UI).
- **Safety:** Dry-run responses and confirm tokens for destructive/costly actions.
- **Persistence:** Store chat threads in hub persistence, not project storage.
- **Handoff:** One tool to open a project-scoped agent session for in-project tasks.
...
| ------------- | ---------------------------------- | ---------------------------------- |
| Projects      | list/create/rename/archive         | not applicable                     |
...
| Remote sync   | configure sync to remote project   | configure sync to remote project   |
| Handoff       | open project agent                 | open project agent                 |

## Handoff pattern example (filesystem or code execution)

User intent: "Clone repo X, create a `codex.chat`, and open it."

1. Control agent validates context and selects the target project (or asks).
2. Control agent calls `handoff_to_project_agent` with:
   - target project id
   - brief task summary (clone repo, create file, open it)
   - optional parameters (repo URL, target path)
3. Project agent performs filesystem actions and reports back.
4. Control agent posts the result in the control-plane chat and links to the opened file or project view.
...
3. **Implement hub tool handlers (minimal set)**
   - **Full hub:** `project.list`, `project.create`, `project.rename`, `project.archive`
   - **Full hub:** `project.add_collaborator`, `project.remove_collaborator`
   - **Full hub:** `project.tag` (or `project.pin`) to cover basic organization
   - **Lite hub:** `logs.search`, `config.get`, `config.set`, `editors.list`, `sync.configure`
   - Stubs for destructive/costly actions (e.g., delete project, start host) that return confirmation requirements.
...
5. **Chat integration using existing UI**
   ...
   - Back this thread with hub persistence (not project storage).
...
6. **Client action proxy (allowlisted UI actions)**
   ...
   - Allowlist actions: open project, switch panel/tab, highlight UI element.
...
8. **Handoff to project agents**
   - Add a `handoff_to_project_agent` tool that opens ACP for a chosen project.
