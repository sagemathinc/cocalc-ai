...
- `bay-0` is configured as the seed bay and `bay-1` as an attached bay.
- `/etc/cocalc/bay-topology.env` is generated by
  `src/scripts/bay-systemd/bay-cluster.sh install-topology`.
- Authenticated peer health works both directions on private IPs using port
  `9402`.
- The attached bay registers with the seed `cluster_bay_registry` over the
  seed Conat fabric at `http://10.206.0.21:10300`.
- Hub workers can be rolling-restarted by the topology installer with
  `--restart-hub-workers` so new topology and inter-bay fabric env takes
  effect.

Remaining known gaps:

- The GCP firewall rule for peer health still needs project-level permission to
  create; current connectivity works because the existing VPC rules allow it.
- Project/account/host ownership routing has not been validated end-to-end on
  Bella.
- Cloudflare public ingress for Bella still needs manual Cloudflare/GCP
  configuration.

The cluster now has a minimal private control-plane transport. The next work is
to prove ownership-aware operations and failure behavior on top of it.
