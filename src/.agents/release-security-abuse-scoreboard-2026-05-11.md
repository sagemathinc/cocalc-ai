...
| SEC-ACP-002     | Codex/ACP durable turn scheduling            | done   | critical | Project-host-local admission now bounds queued, created, and running ACP jobs before normal enqueue/claim. Project-host now overlays cached actor-account membership/admin limits when an ACP request has a trusted account id, falls back to project-owner limits for project-only contexts, records central denial events, and exposes an admin/CLI denial report.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | Add an in-product abuse dashboard or automatic alerting only if CLI/Prometheus denial reporting is not sufficient.                                                                          |
| SEC-ACP-003     | ACP automation scheduling                    | done   | high     | Manual/scheduled automation runs now use the same local ACP admission helper, count against ACP turn caps, and are also bounded by `acp_max_active_automations_per_project`. Denied automations are paused, surfaced in thread automation state, and recorded through the same central ACP denial telemetry used by chat/resend/claim paths. CLI/Prometheus denial reporting is accepted for launch.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | Add an in-product abuse dashboard or automatic alerting only if operators need UI visibility beyond CLI/Prometheus reporting.                                                               |
...
- Project-host installs a local ACP admission limit provider backed by
  host-authenticated effective-limit caches. Lite keeps the env defaults; hosted
  project-host overlays actor-account membership/admin limits when an ACP
  request has a trusted `account_id`, and falls back to project-owner limits for
  project-only contexts.
- Account effective-limit lookup routes to the account home bay, checks product
  access trust on the authoritative bay, caches results on project-host, and
  returns zero ACP limits for untrusted accounts.
...
- Denial observability is centralized and queryable, but there is not yet an
  in-product abuse dashboard or automatic alerting on repeated denials. CLI and
  Prometheus reporting are accepted for launch unless operators need dashboard
  visibility.
- Add per-project 5-hour/7-day worker-start or project wake budget only if load
...
- Central ACP denial telemetry and the CLI/admin denial report exist, but there
  is not yet a broader in-product abuse/account usage dashboard. CLI and
  Prometheus denial reporting are accepted for launch; add a dashboard or alerts
  only if operators need UI visibility.
...
- Passport dependencies and legacy organization-provider machinery still exist
  ...
  concrete direct OIDC replacement is required. This is deferred because Google
  OIDC and organization SAML cover the launch security requirement.

Residual follow-up:

- Add SSO audit events for provider/domain-policy mutations and SSO allow/deny
  outcomes without logging assertions, authorization codes, or provider secrets
  if operators need deeper SSO-specific audit trails.
...
- Membership usage status now exposes current RootFS count, total storage,
  remaining slots/storage, over-limit state, and per-image cap. The account
  membership page and admin user membership diagnostics display the same current
  RootFS usage beside the effective limits.

Residual risk:

- Exact concurrent create/publish serialization is not implemented beyond the
  current admission checks. This is accepted for launch unless telemetry shows
  quota races or abuse.
...
- Monitor scanner-prep status from project-host heartbeat metadata so live hosts
  do not silently miss the Trivy image/cache requirement.
...
- Regenerated `uv.lock`; it now contains only:
  ...
  - `pytest 9.0.3`
  - `Pygments 2.20.0`
- GitHub Dependabot was verified clean by the user on 2026-05-17.

Residual policy:

- Keep normal dependency monitoring enabled.
