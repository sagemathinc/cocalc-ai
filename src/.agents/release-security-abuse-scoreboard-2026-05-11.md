...
| SEC-CLI-001     | `cocalc-cli` authority classes               | guarded | high     | First-pass authority matrix completed. CLI auth config and daemon runtime storage now force private local permissions; ambient env auth can be disabled per invocation. Second-pass dangerous-operation audit completed in `src/.agents/cli-api-key-dangerous-operation-audit-2026-05-14.md`; CLI freshness transport exists via forwarded `auth_session_hash` and `cocalc auth elevate`, but several destructive/admin endpoint families still lack server-side fresh-auth gates.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | Implement or explicitly accept fresh-auth gaps for account delete/rehome, admin membership/entitlement, host delete/rootfs/SSH mutation, project hard delete/move/rehome/restore, rootfs admin mutation, and org token lifecycle. |
...
- Found remaining destructive/admin endpoint families that currently rely on
  ownership/admin/collaborator authorization without fresh auth.

Residual risk:

- Fresh-auth gaps remain for account delete/rehome, admin
  membership/entitlement changes, host delete/rootfs/SSH-authorized-key
  mutation, project hard delete/move/rehome/backup-restore/snapshot-restore,
  rootfs admin mutation, and organization token lifecycle.
...
1. Add a shared Conat endpoint helper for dangerous session auth and apply it to
   the prioritized gaps above, or explicitly accept the residual risk for first
   release.
