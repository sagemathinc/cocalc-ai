...
| SEC-CLI-001     | `cocalc-cli` authority classes               | guarded | high     | First-pass authority matrix completed. CLI auth config and daemon runtime storage now force private local permissions; ambient env auth can be disabled per invocation. Second-pass dangerous-operation audit completed in `src/.agents/cli-api-key-dangerous-operation-audit-2026-05-14.md`; CLI freshness transport exists via forwarded `auth_session_hash` and `cocalc auth elevate`. First fresh-auth implementation tranche now gates account delete/rehome/drain/repair, admin membership/entitlement mutation, and org token lifecycle with a shared dangerous-session helper.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | Implement or explicitly accept remaining fresh-auth gaps for host delete/rootfs/SSH mutation, project hard delete/move/rehome/restore, backup/snapshot mutation, rootfs admin mutation, organization membership mutation, and legacy auth-token generation. |
...
- Fresh-auth gaps remain for host delete/rootfs/SSH-authorized-key mutation,
  project hard delete/move/rehome/backup-restore/snapshot-restore, rootfs admin
  mutation, organization membership/admin mutation if treated as equivalent to
  token issuance, and legacy auth-token generation.
