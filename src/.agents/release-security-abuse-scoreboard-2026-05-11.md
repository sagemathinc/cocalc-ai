...
- Domain policy exists and is used for SSO-required routing, but
  domain-specific `require_cocalc_2fa` and `signup_mode` are not yet enforced.
...
1. Enforce domain-level `require_cocalc_2fa` and `signup_mode`.
2. Replace remaining non-Google organization Passport paths with direct SAML/OIDC
   where needed.
