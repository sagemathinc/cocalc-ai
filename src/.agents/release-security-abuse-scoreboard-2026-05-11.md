...
| SEC-CLI-001     | `cocalc-cli` authority classes               | guarded | high     | First-pass authority matrix completed. CLI auth config and daemon runtime storage now force private local permissions; ambient env auth can be disabled per invocation. Second-pass dangerous-operation audit completed in `src/.agents/cli-api-key-dangerous-operation-audit-2026-05-14.md`; CLI freshness transport exists via forwarded `auth_session_hash` and `cocalc auth elevate`. Fresh-auth implementation now gates account delete/rehome/drain/repair, admin membership/entitlement mutation, org token lifecycle, host delete/deprovision, host RootFS mutation, and host SSH authorized-key mutation with the shared dangerous-session helper. Host SSH authorized-key operations now route to authoritative host bays.                                                                                                                                                                                                                                                                                                                                                                                                   | Implement or explicitly accept remaining fresh-auth gaps for project hard delete/move/rehome/restore, backup/snapshot mutation, rootfs admin mutation, organization membership mutation, and legacy auth-token generation. |
...
- Added a shared Conat dangerous-session helper and applied it to account
  ...
  token lifecycle. Admin and account-rehome/entitlement/token-creation paths
  require active plus recent 2FA; self account delete and org token expiry
...
- Remaining destructive/admin endpoint families still rely on
  ownership/admin/collaborator authorization without fresh auth.

Residual risk:

- Fresh-auth gaps remain for project hard
  delete/move/rehome/backup-restore/snapshot-restore, rootfs admin mutation,
  organization membership/admin mutation if treated as equivalent to token
  issuance, and legacy auth-token generation.
...
1. Apply the shared Conat dangerous-session helper to the remaining prioritized
   gaps above, or explicitly accept the residual risk for first release.
