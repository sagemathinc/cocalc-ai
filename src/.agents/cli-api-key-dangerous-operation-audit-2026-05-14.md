...
- Several high-risk endpoint families now require freshness: membership
  ...
  and RootFS catalog/release admin mutation.
- The remaining destructive/admin gaps are concentrated in organization
...
1. Organization membership/admin mutation if treated as equivalent to
...
- `SEC-CLI-001`: keep `guarded`; the endpoint-level audit is complete, but the
  remaining organization membership/admin mutation freshness gap should be
  fixed or explicitly accepted for first release.
