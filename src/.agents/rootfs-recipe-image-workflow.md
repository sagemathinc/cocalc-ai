...
Recipes are plain data in this repo, not a separate repository:
...
## Authoring: Prefer A Module Over Inline YAML

Inline `run:` steps take literal strings only; there is no include mechanism.
Any script of real length becomes an unreadable heredoc inside YAML. Put real
work in a module `install.sh` so it stays a shell file that can be linted, run,
and diffed.

A module has exactly one `run` script. When one piece of software needs both a
from-scratch build and a periodic update, give the module a `mode` input and
dispatch inside `install.sh`:
...
Make the mode explicit rather than auto-detecting an existing installation.
Recipes also run into live projects via `recipe run --here`, and a script that
silently switches behavior based on what it finds is surprising in that
context.

Then keep two thin image specs that share the module's contributed catalog
metadata (family, tags, theme):

- `examples/texlive.yaml` — `with: { mode: install }`
- `examples/texlive-update.yaml` — `with: { mode: update }`

Anything site-specific (mirrors, internal paths) belongs in recipe **inputs**
with sensible public defaults, never hardcoded in `install.sh`. These files are
committed and ship inside the CLI for all users.
