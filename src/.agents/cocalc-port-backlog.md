...
| #8693 | knitr time-travel frame path                                                           | **Implemented in `knitr-time-travel-20260907`, pending merge.** Port the `set_frame_type` override so selecting TimeTravel uses the original rnw/rtex source instead of generated tex. Regression coverage includes repeated frame switching and ordinary LaTeX. |
