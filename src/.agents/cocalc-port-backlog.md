...
| #8723 | Email-address field keeps edited value after Cancel                                    | **Implemented in PR #426.** Cancel restores the saved email prop (or empty string). Keyboard regression coverage reproduces the stale displayed address before the fix. Port includes state reset and controlled input; saving locks the form until the request settles. The existing Card and fresh-auth flow remain. |
