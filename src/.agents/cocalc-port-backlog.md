...
| #8723 | Email-address field keeps edited value after Cancel                                    | **Implemented in `email-cancel-fix-20260907`, pending review/merge.** Cancel restores the saved email prop (or empty string). Keyboard regression coverage reproduces the stale displayed address before the fix. Port includes state reset and controlled input; saving locks the form and times out after 30 seconds. The existing Card and fresh-auth flow remain. |
