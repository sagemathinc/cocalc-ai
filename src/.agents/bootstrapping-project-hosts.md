...
### 1.1b Dependency strategy (runtime vs CI)

- **Runtime**: stdlib-only Python. No pip. No external deps.
- **CI/Dev**: optional deps (pydantic/mypy/ruff) for validation and type checking.
- **If we ever need a dependency at runtime**:
  - Prefer vendoring a single-file module into the repo, or
  - Ship a self-contained zipapp that bundles deps (still no pip).

### 1.1c Bootstrap distribution strategy

- **Dedicated bootstrap versioning** in R2 (not bundled in the hub):
  - `software/bootstrap/<version>/bootstrap.py`
  - `software/bootstrap/<version>/bootstrap.py.sha256`
- **Channels** (server-side mapping):
  - `bootstrap/latest` → stable version
  - `bootstrap/test` → candidate version
- **Config selection**:
  - Site setting default: `project_hosts_bootstrap_channel` (default `latest`)
  - Optional site pin: `project_hosts_bootstrap_version`
  - Per-host override via metadata: `bootstrap_channel` or `bootstrap_version`
- **Shell stub behavior**:
  - Download the exact version+sha256 from R2.
  - Fail fast on checksum mismatch.
  - Hub endpoint `/project-host/bootstrap.py` remains as a debug fallback.
