...
// Minimal settings required to evaluate an LLM request. Concrete callers
// (server vs lite) provide these so this package does not import database/server.
export interface LLMSettings {
  openai_api_key?: string;
  google_vertexai_key?: string;
...
export async function evaluateWithLangChain(
  ...
  const client = await config.createClient(options, context);
