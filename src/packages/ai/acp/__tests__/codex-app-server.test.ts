...
describe("CodexAppServerAgent", () => {
  ...
  it("preserves a retained incompatible runtime with background work", async () => {
    const methods: string[] = [];
    const proc = new FakeCodexAppServerProc((fake, message) => {
      methods.push(message.method);
      switch (message.method) {
        case "thread/start":
          fake.sendResponse(message.id, { thread: { id: "retained-thread" } });
          break;
        case "turn/start":
          fake.sendResponse(message.id, { turn: { id: "retained-turn" } });
          setImmediate(() =>
            fake.sendNotification("turn/completed", {
    ...
    try {
      await agent.evaluate({ ...request, prompt: "start background work" });
      const events: any[] = [];
      await agent.evaluate({
        ...request,
        prompt: "message a selected agent",
        mentionReferences: [
          {
            version: 1,
            naming_account_id: "Q",
      ...
      expect(events).toContainEqual({
        type: "error",
        error: expect.stringContaining("preserved its current runtime"),
      });
    } finally {
      await agent.dispose();
    }
  ...
  it("does not charge a later timeout against the refresh retry budget", async () => {
    process.env.COCALC_CODEX_TIMEOUT_MAX_RETRIES = "1";
    process.env.COCALC_CODEX_TIMEOUT_RETRY_DELAY_MS = "1000";
    const dir = mkdtempSync(path.join(tmpdir(), "acp-refresh-timeout-"));
    const processes: FakeCodexAppServerProc[] = [];
    const spawnCodexAppServer = jest.fn(async () => {
      const spawnIndex = processes.length;
      const identityPath = path.join(dir, `identity-${spawnIndex}.json`);
      ...
      const proc = new FakeCodexAppServerProc((fake, message) => {
        switch (message.method) {
          case "thread/start":
            fake.sendResponse(message.id, { thread: { id: "retry-thread" } });
            break;
          case "turn/start": {
            const turnId = `retry-turn-${spawnIndex}`;
            fake.sendResponse(message.id, { turn: { id: turnId } });
            setImmediate(() => {
              if (spawnIndex === 1) {
                fake.sendNotification("turn/completed", {
      ...
      expect(
        events.filter(
          (event) =>
            ...
            event.event.text.includes("transient timeout"),
        ),
      ).toHaveLength(1);
    } finally {
      await agent.dispose();
