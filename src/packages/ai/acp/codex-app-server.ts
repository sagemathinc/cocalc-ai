...
function getCoCalcProjectRuntimeGuidance(cliCommand: string): string[] {
  return [
    ...
    `For a requested or opted-in chat workbench workflow, inspect \`${cliCommand} project chat artifact --help\` and the cocalc skill. Supported artifacts: collaborative Markdown, file previews, proposed action reviews, and GitHub PR cards. Publish useful persistent results through this API; CoCalc handles cards and native frame tabs. If unavailable, report that and use normal text/file links, never direct .chat writes or arbitrary HTML/apps.`,
