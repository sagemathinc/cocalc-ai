...
function getCoCalcRuntimeGuidanceHeader(cliCommand: string): string {
  return [
    ...
    "For notebook edits/execution that must survive browser refresh or disconnect, prefer `cocalc project jupyter -h` over `browser exec`; use `browser exec` only for UI-only notebook context such as selection or viewport state.",
