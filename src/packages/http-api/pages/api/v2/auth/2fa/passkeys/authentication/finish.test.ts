...
const finishSignInPasskeyAuthenticationMock = jest.fn();
const completeEmailAuthMfaMock = jest.fn();
const signUserInMock = jest.fn();

jest.mock("@cocalc/server/auth/passkeys", () => ({
  finishSignInPasskeyAuthentication: (...args: any[]) =>
    finishSignInPasskeyAuthenticationMock(...args),
}));
...
describe("/api/v2/auth/2fa/passkeys/authentication/finish", () => {
  beforeEach(() => {
    finishSignInPasskeyAuthenticationMock.mockReset().mockResolvedValue({
  ...
  it("does not sign in when authoritative email MFA completion fails", async () => {
    ...
    const { default: handler } = await import("./finish");
