...
import { renamePasskey } from "@cocalc/server/auth/passkeys";

export default async function renamePasskeyApi(req, res) {
  try {
    ...
    const { factor_id, label } = getParams(req);
    res.json(
      await renamePasskey({
        req,
        account_id,
    ...
    res.json({
      error: err instanceof Error ? err.message : "Problem renaming passkey.",
    });
  }
}
