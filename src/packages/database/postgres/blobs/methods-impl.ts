...
export function save_blob(db: PostgreSQLType, opts: SaveBlobOpts) {
  ...
  const dbg = db._dbg(`save_blob(uuid='${optsNormalized.uuid}')`) as (
    ...args: unknown[]
  ) => void;
  dbg();
...
export function _extend_blob_ttl(db: PostgreSQLType, opts: ExtendBlobTtlOpts) {
  ...
  if (optsWithDefaults.ttl) {
    ...
    if (z > optsWithDefaults.expire) {
      ...
      ttl = (optsWithDefaults.expire.getTime() - new Date().getTime()) / 1000.0;
    }
  } else {
...
export function get_blob(db: PostgreSQLType, opts: GetBlobOpts) {
  const optsWithDefaults = defaults(opts, {
    uuid: required,
    ...
    [
      ...
      (cb: CB) => {
        ...
        switch (compression) {
          ...
          default:
            return cb(`compression format '${compression}' not implemented`);
        }
      },
    ],
    (err) => {
      ...
      if (blob != null && optsWithDefaults.touch) {
        ...
        return db.touch_blob({ uuid: optsWithDefaults.uuid });
      }
    },
  );
}

export function touch_blob(db: PostgreSQLType, opts: TouchBlobOpts) {
  const optsWithDefaults = defaults(opts, {
    uuid: required,
    cb: undefined,
  }) as TouchBlobOpts;
...
export function blob_store(db: PostgreSQLType, bucket?: string): BlobStore {
  void db;
  if (!bucket) {
...
export function copy_blob_to_gcloud(
  db: PostgreSQLType,
  opts: CopyBlobToGcloudOpts,
) {
  const optsWithDefaults = defaults(opts, {
  ...
  return async.series(
    [
      ...
      (cb) => {
        ...
        locals.bucket = blob_store(db, optsWithDefaults.bucket);
...
*/
export function backup_blobs_to_tarball(
  db: PostgreSQLType,
  opts: BackupBlobsToTarballOpts,
) {
  const optsWithDefaults = defaults(opts, {
    ...
    (err) => {
      if (err) {
        ...
        if (
          ...
          const f = () => {
            return db.backup_blobs_to_tarball(optsWithDefaults);
          };
...
*/
export function copy_all_blobs_to_gcloud(
  db: PostgreSQLType,
  opts: CopyAllBlobsToGcloudOpts,
) {
  const optsWithDefaults = defaults(opts, {
    bucket: COCALC_BLOB_STORE,
  ...
  const dbg = db._dbg("copy_all_blobs_to_gcloud") as (
    ...args: unknown[]
  ) => void;
  dbg();
  ...
  return db._query({
    ...
    cb: all_results((err, rows: BlobSizeRow[]) => {
      if (err) {
        dbg(`fail: ${err}`);
        const blobErr =
          err instanceof Error || typeof err === "string" ? err : String(err);
        ...
        return async.mapLimit(rows, optsWithDefaults.map_limit, f, (err) => {
          ...
          if (optsWithDefaults.repeat_until_done_s && rows.length > 0) {
            ...
            return setTimeout(
              () => db.copy_all_blobs_to_gcloud(optsWithDefaults),
...
export async function blob_maintenance(
  db: PostgreSQLType,
  opts: BlobMaintenanceOpts,
): Promise<void> {
  const {
    path = "/backup/blobs",
    map_limit = 1,
  ...
  const dbg = db._dbg("blob_maintenance()") as (...args: unknown[]) => void;
  dbg();

  try {
    // Step 1: Maintain the patches and syncstrings
    dbg("maintain the patches and syncstrings");
    await callback_opts(db.syncstring_maintenance.bind(db))({
    ...
    await callback_opts(db.backup_blobs_to_tarball.bind(db))({
      throttle,
    ...
    await callback_opts(db.copy_all_blobs_to_gcloud.bind(db))({
      limit: 1000,
...
export function remove_blob_ttls(db: PostgreSQLType, opts: RemoveBlobTtlsOpts) {
  const optsWithDefaults = defaults(opts, {
...
export function close_blob(db: PostgreSQLType, opts: CloseBlobOpts) {
  const optsWithDefaults = defaults(opts, {
 ...
 */
export function syncstring_maintenance(
  db: PostgreSQLType,
  opts: SyncstringMaintenanceOpts,
) {
  const optsWithDefaults = defaults(opts, {
  ...
  const dbg = db._dbg("syncstring_maintenance") as (...args: unknown[]) => void;
  ...
  return async.series(
    [
      ...
      (cb) => {
        ...
        const f = (string_id: string, cb: CB) => {
          i += 1;
          console.log(`*** ${i}/${total}: archiving string ${string_id} ***`);
          return db.archivePatches({
            string_id,
            cb: (err) => {
    ...
    (err) => {
      if (err) {
        ...
        dbg("doing it again");
        return db.syncstring_maintenance(optsWithDefaults);
      } else {
        return optsWithDefaults.cb?.();
      }
    },
...
export async function archivePatches(
  db: PostgreSQLType,
  opts: ArchivePatchesOpts,
) {
  try {
    const opts_with_db: ArchivePatchesOptsWithDb = { ...opts, db };
...
*/
export async function export_patches(
  db: PostgreSQLType,
  opts: ExportPatchesOpts,
) {
  void db;
  try {
...
export async function import_patches(
  db: PostgreSQLType,
  opts: ImportPatchesOpts,
): Promise<void> {
  const optsWithDefaults = defaults(opts, {
  ...
  for (const patch of normalizedPatches) {
    if (patch.is_snapshot == null) {
      const err = "is_snapshot is required";
      optsWithDefaults.cb?.(err);
      if (!optsWithDefaults.cb) {
        throw new Error(err);
      }
      return;
    }
  ...
  const totalBlocks = Math.ceil(normalizedPatches.length / insert_block_size);
  const insertBlock = (i: number) =>
...
export function delete_blob(db: PostgreSQLType, opts: DeleteBlobOpts) {
  const optsWithDefaults = defaults(opts, {
    uuid: required,
    cb: undefined,
  }) as DeleteBlobOpts;
  ...
  const dbg = db._dbg(`delete_blob(uuid='${optsWithDefaults.uuid}')`) as (
    ...args: unknown[]
  ) => void;
  return async.series(
    [
      ...
      (cb) => {
        ...
        return blob_store(db, gcloud).delete({
          name: optsWithDefaults.uuid,
          cb,
        });
      },
