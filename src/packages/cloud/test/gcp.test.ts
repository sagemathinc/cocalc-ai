...
describe("GcpProvider", () => {
  ...
  it("creates a VM with a provider-enforced STOP run limit", async () => {
    insertMock.mockResolvedValueOnce([
      { latestResponse: { name: "op-limited", status: "DONE" } },
    ]);
    waitMock.mockResolvedValueOnce([{ status: "DONE" }]);
    getMock.mockResolvedValueOnce([
      {
    ...
    const provider = new GcpProvider();
    const runtime = await provider.createHost(
      buildSpec({ metadata: { max_run_duration_seconds: 86_400 } }),
      {
        project_id: "proj-1",
        client_email: "svc@example.com",
        private_key: "key",
    ...
    expect(
      insertMock.mock.calls[0][0].instanceResource.scheduling,
    ).toMatchObject({
      maxRunDuration: { seconds: 86_400 },
      instanceTerminationAction: "STOP",
    });
    expect(runtime.metadata?.max_run_duration_seconds).toBe(86_400);
  });

  it("stops and repairs a legacy running VM before sponsored restart", async () => {
    getMock
      .mockResolvedValueOnce([{ status: "RUNNING", scheduling: {} }])
      .mockResolvedValueOnce([{ status: "TERMINATED", scheduling: {} }])
