...
export function configureProjectHostServiceAdmissionDenialRecorder(): void {
  const forward = async (
    ...
    if (!client || !host_id || !project_id) {
      logger.debug("skipping service admission telemetry", {
        has_client: !!client,
        has_host_id: !!host_id,
  ...
  setServiceAdmissionDenialRecorder(
    async (event: ServiceAdmissionDenialEvent) =>
      await forward("hosts.recordServiceAdmissionDenial", event),
  );
  setServiceAdmissionNearLimitRecorder(
    async (event: ServiceAdmissionDenialEvent) =>
      await forward("hosts.recordServiceAdmissionNearLimit", event),
  );
}
