...
function resolveStorageMount(): string {
...
async function runCommand(
  ...
  try {
    ...
    if (result.exit_code !== 0) {
      logger.debug("host metrics command failed", {
        command,
        args,
    ...
    logger.debug("host metrics command errored", {
      command,
      args,
      err: `${err}`,
    });
...
function parseBtrfsUsageOutput(output: string): Partial<HostCurrentMetrics> {
...
function parseDfOutput(output: string): Partial<HostCurrentMetrics> {
...
async function collectSnapshot(
  ...
  const mount = resolveStorageMount();
  const [memory, disk] = await Promise.all([
    readMeminfo(),
    readDiskMetrics(mount),
  ]);
  const projects = readProjectCounts();
  const reservation_bytes = 0;
