...
test("buildCookieHeader includes account_id with hub password impersonation", () => {
  const accountId = "00000000-1000-4000-8000-000000000056";
  const header = buildCookieHeader(
    "https://lite2.cocalc.ai",
    {
      hubPassword: "dev-hub-password",
      accountId,
  ...
  assert.ok(
    header?.includes(
      `${cookieNameFor("https://lite2.cocalc.ai", "account_id")}=${accountId}`,
    ),
  );
});
