...
export function registerProjectSyncCommands(
  ...
  const sync = project.command("sync").description("project sync and forwarding operations");

  const syncKey = sync.command("key").description("manage ssh keys for project sync");
  ...
  syncKey
    .command("install")
    .description("install a local ssh public key into project .ssh/authorized_keys")
  ...
  syncForward
    ...
    .option("--install-key", "install local ssh public key into project before creating forward")
    .option("--key-path <path>", "ssh key base path (default: ~/.ssh/id_ed25519)")
    .action(
      async (
        opts: {
        ...
        await withContext(command, "project sync forward create", async (ctx) => {
  ...
  syncForward
    ...
    .option("-w, --project <project>", "project id or name (defaults to context)")
    .option("--all", "list all local forwards (ignore project context)")
    .action(async (opts: { project?: string; all?: boolean }, command: Command) => {
      if (opts.all) {
        await runLocalCommand(command, "project sync forward list", async () => {
          const rows = await listReflectForwards();
      ...
      await withContext(command, "project sync forward list", async (ctx) => {
        const target = await resolveProjectSshTarget(ctx, opts.project);
        const rows = await listReflectForwards();
        return forwardsForProject(rows, target.project.project_id).map((row) =>
  ...
  syncForward
    ...
    .action(
      async (
        ...
        if (refs.length > 0) {
          await runLocalCommand(command, "project sync forward terminate", async () => {
        ...
        await withContext(command, "project sync forward terminate", async (ctx) => {
          const target = await resolveProjectSshTarget(ctx, opts.project);
          const rows = forwardsForProject(await listReflectForwards(), target.project.project_id);
          const ids = rows.map((row) => String(row.id));
          await terminateReflectForwards(ids);
          return {
            project_id: target.project.project_id,
