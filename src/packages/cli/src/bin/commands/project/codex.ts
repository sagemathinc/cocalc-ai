...
export function registerProjectCodexCommands(
  ...
  const codex = project
    .command("codex")
    .description("project codex operations");

  codex
    .command("exec [prompt...]")
    .description(
      "run a codex turn in a project using project-host containerized codex exec",
    )
    ...
    .action(
      async (
        ...
        await withContext(command, "project codex exec", async (ctx) => {
          ...
          const streamJsonl =
            !!opts.jsonl || (!!opts.stream && wantsJsonOutput);
          const streamHuman =
            !streamJsonl && (!!opts.stream || !!ctx.globals.verbose);
  ...
  const codexAuth = codex
    .command("auth")
    .description("project codex authentication");

  codexAuth
    .command("status")
    .description(
      "show effective codex auth/payment source status for a project",
    )
    .option("-w, --project <project>", "project id or name")
  ...
  codexAuthSubscription
    .command("login")
    .description(
      "start device auth login flow (waits for completion by default)",
    )
    .option("-w, --project <project>", "project id or name")
  ...
  codexAuthSubscription
    ...
    .option("-w, --project <project>", "project id or name")
    .action(
      async (opts: { id: string; project?: string }, command: Command) => {
        await withContext(
          command,
  ...
  codexAuthApiKey
    ...
    .option("-w, --project <project>", "project id or name (for project scope)")
    .action(
      async (opts: { scope?: string; project?: string }, command: Command) => {
        await withContext(
          command,
