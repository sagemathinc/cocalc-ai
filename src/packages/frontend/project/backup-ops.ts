/**
 * Backup-ops LRO UI state manager: tracks active backup operations for a project
...
export type BackupLroState = LroOpState;
...
export class BackupOpsManager {
  private manager: MultiLroOpsManager;

  constructor(opts: BackupOpsManagerOptions) {
    this.manager = new MultiLroOpsManager({
      ...
      log: opts.log,
      setState: (state) =>
        opts.setState({ backup_ops: state ? ImmutableMap(state) : undefined }),
    });
  }
