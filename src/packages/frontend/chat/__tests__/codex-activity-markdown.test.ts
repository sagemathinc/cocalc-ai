...
describe("codexEventsToMarkdown", () => {
  it("joins chunked adjacent agent messages without inventing paragraph breaks", () => {
    const markdown = codexEventsToMarkdown([
      {
        ...
        event: {
          type: "message",
          text: "I am going to inspect the docs action",
        },
      } as any,
      {
        type: "event",
        ...
        event: {
          type: "message",
          text: "system and patch the live output renderer.",
        },
      } as any,
    ]);

    expect(markdown).toContain(
      "- Agent: I am going to inspect the docs action system and patch the live output renderer.",
    );
    expect(markdown).not.toContain("docs action\n\nsystem");
  });

  it("keeps intentional paragraph boundaries for markdown section chunks", () => {
    const markdown = codexEventsToMarkdown([
      {
        type: "event",
        seq: 1,
      ...
      } as any,
      {
        ...
        event: {
          type: "message",
          text: "**Next step**: patch the renderer.",
        },
      } as any,
    ]);

    expect(markdown).toContain(
      "- Agent: I found the issue.\n\n**Next step**: patch the renderer.",
    );
  });

  it("preserves explicit blank lines across adjacent message chunks", () => {
    const markdown = codexEventsToMarkdown([
      {
        type: "event",
        seq: 1,
