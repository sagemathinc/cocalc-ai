...
test("publication card opens an additional frame with explicit origin and snapshot", () => {
  const records: any[] = [];
  ...
  const split_frame = jest.fn();
  const actions: any = {
    syncdb,
    frameId: "chat-frame",
    frameTreeActions: { split_frame, get_frame_ids_in_order: () => [] },
  };
  render(
    <ArtifactCards
      actions={actions}
