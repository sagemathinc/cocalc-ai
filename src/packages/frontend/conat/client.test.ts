...
describe("ConatClient routed project-host reconnect", () => {
  ...
  it("does not privately resolve host info when reconnecting a public directory share", async () => {
    ...
    expect(ensureHostInfo).not.toHaveBeenCalled();
    expect(hubRequest).not.toHaveBeenCalledWith(
      "hub.account.acct-1.api",
      {
        name: "hosts.resolveHostConnection",
        args: [{ host_id: "host-1" }],
  ...
  it("keeps routed host clients alive during a hub reconnect", () => {
    const hubClient = {
      inboxPrefixHook: undefined,
      info: undefined,
      conn: {
    ...
    expect(routedClient.close).not.toHaveBeenCalled();
    expect(client.routedHubClients["host-1"]?.client).toBe(routedClient);
  ...
  it("does not fall back to the hub for projectFs in launchpad mode when host routing is unavailable", async () => {
    const ensureHostInfo = jest.fn(async () => undefined);
    const hubFs = jest.fn(() => {
      throw new Error("hub fs should not be used");
    });
    ...
    await expect(
      client.projectFs({
        project_id: "00000000-1000-4000-8000-000000000000",
        caller: "test.projectFs",
      }),
    ).rejects.toThrow("unable to route 'test.projectFs' to project-host");
    expect(hubFs).not.toHaveBeenCalled();
  });
  ...
  it.each([
    { name: "lite", liteMode: true, runtimeMode: "external" },
    { name: "workspace", liteMode: false, runtimeMode: "workspace" },
  ])(
    "does not require project-host routing for project clients in $name mode",
    async ({ liteMode, runtimeMode }) => {
      const ensureHostInfo = jest.fn(async () => undefined);
      const hubClient = {
        inboxPrefixHook: undefined,
        info: undefined,
      ...
      jest.doMock("@cocalc/frontend/app-framework", () => ({
        redux: {
          getStore: jest.fn((name: string) => {
            if (name === "customize") {
              return immutable.Map({
                project_runtime: immutable.Map({ mode: runtimeMode }),
              });
            }
            if (name !== "projects") return undefined;
            return immutable.Map({
...
describe("ConatClient main reconnect scheduling", () => {
  ...
  it("probes routed project hosts after wake and recovers resources in place", async () => {
    jest.resetModules();
    Object.defineProperty(window.navigator, "onLine", {
      configurable: true,
      value: true,
    ...
    try {
      ...
      class MockCoreClient extends EventEmitter {
        ...
        constructor(public readonly address: string) {
          super();
          this.conn = new EventEmitter();
          this.conn.connected = address === "http://hub";
          this.conn.connect = this.connect;
          this.conn.on = this.conn.addListener.bind(this.conn);
          this.conn.io = {
            on: jest.fn(),
          ...
          this.request =
            address === "http://project-host"
              ? jest.fn(async () => {
                  throw Error("project host wake probe failed");
                })
              : jest.fn(async () => ({ data: { pong: true } }));
        }
      }
      ...
      jest.doMock("@cocalc/frontend/app-framework", () => ({
        redux: {
          getStore: jest.fn((name: string) => {
            if (name !== "projects") return undefined;
            return immutable.Map({
              open_projects: immutable.List([projectId]),
              project_map: immutable.Map({
                [projectId]: immutable.Map({
                  host_id: "host-1",
                  owning_bay_id: "bay-1",
                }),
              }),
      ...
      jest.doMock("@cocalc/conat/core/client", () => ({
        connect: jest.fn((opts?: any) => {
          const client = new MockCoreClient(opts?.address);
          clients.push(client);
          return client;
        }),
      }));
      ...
      client.registerReconnectResource({
        isConnected: () => true,
        priority: () => "background",
        reconnect: resourceReconnect,
      });
      const hubClient = client.conat();
      hubClient.info = {
        id: "hub-0",
        user: { account_id: "acct-1" },
      };
      ...
      client.projectConatSync({ project_id: projectId, caller: "test" });
      await Promise.resolve();
      await jest.advanceTimersByTimeAsync(100);
      const routedClient = clients.find(
        (candidate) => candidate.address === "http://project-host",
      );
      expect(routedClient).toBeDefined();
      expect(routedClient.connect).toHaveBeenCalledTimes(1);
      ...
      await Promise.resolve();
      await jest.advanceTimersByTimeAsync(100);
      expect(routedClient.disconnect).toHaveBeenCalledTimes(1);
      expect(routedClient.conn.io.engine.close).toHaveBeenCalledTimes(1);
      expect(routedClient.connect).toHaveBeenCalledTimes(2);
  ...
  it("wakes pending resource reconnects when a routed host reconnects", async () => {
    jest.resetModules();
    Object.defineProperty(window.navigator, "onLine", {
      configurable: true,
      value: true,
    ...
    try {
      ...
      class MockCoreClient extends EventEmitter {
        ...
        request = jest.fn(async () => ({ data: { pong: true } }));
      ...
      const resource = client.registerReconnectResource({
        isConnected: () => false,
        priority: () => "foreground",
        reconnect: resourceReconnect,
      });
      ...
      resource.requestReconnect({ reason: "terminal_socket_disconnected" });
      await jest.advanceTimersByTimeAsync(1_000);
      expect(resourceReconnect).toHaveBeenCalledTimes(1);

      hostReady = true;
      routedClient.emit("connected");

      await jest.advanceTimersByTimeAsync(999);
      expect(resourceReconnect).toHaveBeenCalledTimes(1);
      await jest.advanceTimersByTimeAsync(1);
      expect(resourceReconnect).toHaveBeenCalledTimes(2);
    } finally {
      hasFocusSpy.mockRestore();
      randomSpy.mockRestore();
    }
