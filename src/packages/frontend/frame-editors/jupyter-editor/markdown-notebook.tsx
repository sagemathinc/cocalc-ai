...
export function MarkdownNotebookTopLevel(props: Props): React.JSX.Element {
    ...
    return {
      set_value: (markdown) => {
        try {
          ...
          // eslint-disable-next-line no-console
          console.error("Failed to apply top-level notebook markdown", err);
          setError("Could not apply edits to notebook cells.");
        }
      },
      shiftEnter: (markdown) => runCellAtCursor({ markdown, insertBelow: false }),
      altEnter: (markdown) => runCellAtCursor({ markdown, insertBelow: true }),
    };
  }, [read_only, cell_list, cells, applyNotebookMarkdown, runCellAtCursor]);
  ...
  return (
    <div
      ...
      data-cocalc-jupyter-slate-notebook-top-level="1"
    >
      <div style={{ color: "#888", fontSize: "11px", marginBottom: "8px" }}>
        Top-level Slate notebook editor (experimental)
      </div>
      {error ? (
        <div
          style={{
      ...
      <BlockMarkdownEditor
        key="jupyter-slate-top-level-editor"
        value={value}
        actions={editorActions}
        read_only={!!read_only}
        hidePath
