...
export class NotebookFrameActions {
  ...
  */
  ...
  public activate_cell(
    ...
    // Never enter edit mode on a protected cell (metadata.editable === false);
    // activate it in escape mode instead, so it becomes current but stays
    // uneditable.
    const mode =
      opts.mode === "edit" && !store.is_cell_editable(id)
        ? "escape"
        : opts.mode;
