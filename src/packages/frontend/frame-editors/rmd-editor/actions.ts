...
export class Actions extends MarkdownConverterActions {
  ...
  _init2(): void {
    ...
    if (this.is_read_only_preview()) {
      // No builds in read-only previews: skip converter/coordinator init.
      return;
    }
  ...
  protected async _run_converter(
    hash?: number,
    buildToken?: string,
  ): Promise<void> {
    const ownsBuild = () => this.isBuildOwner(buildToken);
    if (!ownsBuild()) return;
    ...
    try {
      ...
      output = await convert(
        ...
        (job) => {
          if (ownsBuild()) this.set_job_info(job);
        },
      );
      if (!ownsBuild()) return;
      this.set_log(output);
