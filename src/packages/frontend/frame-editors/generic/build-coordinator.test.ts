...
// Mock the webapp client — jest.mock is hoisted above imports.
// The coordinator obtains its DKV via webapp_client.conat_client.dkv and
// timestamps via webapp_client.time_client.server_time (through ./client).
jest.mock("@cocalc/frontend/webapp-client", () => ({
  webapp_client: {
    conat_client: {
      // Explicit multibay routing: the coordinator resolves the project's
      // conat client first and passes it to dkv(). The mock just returns a
      // token client object.
      projectConat: async () => ({ mock: "project-conat-client" }),
      dkv: () => dkvPromise,
    },
    time_client: {
...
function makeCallbacks(overrides?: Partial<BuildCoordinatorCallbacks>) {
  ...
  const callbacks: BuildCoordinatorCallbacks = {
    ...
    setBuilding: jest.fn((v: boolean, _buildId: string) => {
      building = v;
    }),
    setError: jest.fn(),
    ...overrides,
...
describe("BuildCoordinator", () => {
  ...
  describe("local lifecycle reconciliation", () => {
    test("joins a replacement start that arrived during a local build", async () => {
      let building = true;
      const cb = makeCallbacks({
        isBuilding: jest.fn(() => building),
        setBuilding: jest.fn((value: boolean) => {
          building = value;
        }),
      });
      const coord = new BuildCoordinator(PROJECT_ID, PATH, cb);
      ...
      coord.setLocalBuildId("local-A");
      coord.publishBuildStart("local-A", 1);
      store.set(PATH, {
        buildId: "remote-B",
        status: "running",
        aggregate: 2,
      ...
      coord.publishBuildFinished("local-A");
      building = false;
      coord.reconcileRunningBuild();

      expect(cb.join).toHaveBeenCalledWith("remote-B", 2, false, undefined);
      coord.close();
    });
  });
  ...
  describe("remote build joining", () => {
    test("remote build start triggers join callback", async () => {
      ...
      expect(cb.join).toHaveBeenCalledWith("remote-1", 42, true, undefined);
      expect(cb.setBuilding).toHaveBeenCalledWith(true, "remote-1");
    ...
    test("remote build finish resets building state after join completes", async () => {
      ...
      expect(cb.setBuilding).not.toHaveBeenCalledWith(false, "r1");
      ...
      expect(cb.setBuilding).toHaveBeenCalledWith(false, "r1");
  ...
  describe("stop propagation", () => {
    test("requestStop transitions running → stopping in DKV", async () => {
      ...
      // The requester already executes stop_build itself; its synchronous
      // DKV echo must not launch a second local kill sequence.
    ...
    test("stopping status triggers stop callback for active build id", async () => {
      ...
      expect(cb.stop).toHaveBeenCalledWith("r1");
  ...
  describe("late joiner", () => {
    test("joins a build that was already running when DKV inits", async () => {
      ...
      expect(cb.join).toHaveBeenCalledWith(
        "already-running",
        77,
        false,
        undefined,
      );
      expect(cb.setBuilding).toHaveBeenCalledWith(true, "already-running");
    ...
    test("passes force flag from existing DKV entry to join", async () => {
      ...
      expect(cb.join).toHaveBeenCalledWith("force-build", 55, true, undefined);
    ...
    test("clears a stale 'running' entry instead of joining (age > 20 min)", async () => {
      ...
      expect(cb.join).not.toHaveBeenCalled();
      expect(cb.setBuilding).not.toHaveBeenCalledWith(true, "stranded");
    ...
    test("still joins a fresh 'running' entry (age < 20 min)", async () => {
      ...
      expect(cb.join).toHaveBeenCalledWith("fresh", 5, false, undefined);
    ...
    test("join completes and resets building state", async () => {
      ...
      expect(cb.setBuilding).toHaveBeenCalledWith(true, "r1");
      ...
      expect(calls[calls.length - 1]).toEqual([false, "r1"]);
  ...
  describe("buffered operations", () => {
    ...
    test("runtime reset during init discards operations from the dead runtime", async () => {
      const cb = makeCallbacks();
      const coord = new BuildCoordinator(PROJECT_ID, PATH, cb);

      coord.setLocalBuildId("dead-build");
      coord.publishBuildStart("dead-build", 200);
      coord.resetRuntimeState();
    ...
    test("runtime reset during init rejects a pre-existing running entry", async () => {
      const cb = makeCallbacks();
      const coord = new BuildCoordinator(PROJECT_ID, PATH, cb);
      mockDkvInstance.set(PATH, {
        buildId: "dead-remote",
    ...
    test("runtime reset during init still joins a newer replacement", async () => {
      const cb = makeCallbacks();
      const coord = new BuildCoordinator(PROJECT_ID, PATH, cb);
      coord.resetRuntimeState();
      mockDkvInstance.set(PATH, {
        buildId: "fresh-remote",
        status: "running",
        aggregate: 11,
        startedAt: Date.now() + 1000,
      });
    ...
    test("runtime reset remains compatible with later timestamp-less clients", async () => {
      const cb = makeCallbacks();
      const coord = new BuildCoordinator(PROJECT_ID, PATH, cb);
      const store = await initDkv();
  ...
  describe("publishBuildFinished edge cases", () => {
    test("entry already gone — clears _localBuildId immediately", async () => {
      ...
      expect(cb.join).toHaveBeenCalledWith("b1", 50, false, undefined);
    ...
    test("entry overwritten by another client — clears _localBuildId", async () => {
      ...
      expect(cb.join).toHaveBeenCalledWith("b1", 77, false, undefined);
  ...
  describe("join lifecycle races (P1 fix)", () => {
    test("build-finished during active join does not clear building state", async () => {
      ...
      expect(cb.join).toHaveBeenCalledWith("r1", 42, false, undefined);
      expect(cb.setBuilding).toHaveBeenCalledWith(true, "r1");
    ...
    test("replacement build published during an active join is joined after the join settles", async () => {
      // A start that arrives while isBuilding() is true is dropped by
      // handleBuildStart.  The finally block of joinBuild must re-read
      // the DKV and join a still-running entry with a different buildId,
      // otherwise the replacement build is permanently missed.
      const joinControl = { resolve: (_v?: unknown) => {} };
      let firstJoin = true;
      const cb = makeCallbacks({
        join: jest.fn(() => {
      ...
      // Remote build A starts (stored, so the settle-time re-read sees data)
      store.set(PATH, { buildId: "rA", status: "running", aggregate: 1 });
      expect(cb.join).toHaveBeenCalledTimes(1);

      // While joining A, build B replaces the entry — dropped for now.
      store.set(PATH, { buildId: "rB", status: "running", aggregate: 2 });
      expect(cb.join).toHaveBeenCalledTimes(1);

      // A's join settles → the coordinator re-reads the DKV and joins B.
      joinControl.resolve();
      await tick();
      expect(cb.join).toHaveBeenCalledTimes(2);
      expect(cb.join).toHaveBeenLastCalledWith("rB", 2, false, undefined);

      coord.close();
    });

    test("join settle does not re-join the same still-running buildId", async () => {
      // If the entry still shows the build we just joined (originator
      // hasn't finished yet), re-joining it could loop forever.
      const joinControl = { resolve: (_v?: unknown) => {} };
      const cb = makeCallbacks({
        join: jest.fn(
          () =>
      ...
      joinControl.resolve();
      await tick();
      // Entry unchanged (same buildId, still "running") — no second join.
      expect(cb.join).toHaveBeenCalledTimes(1);

      coord.close();
    });

    test("runtime reset invalidates an active join and terminalizes its DKV entry", async () => {
      const joinControl = { resolve: (_v?: unknown) => {} };
      const cb = makeCallbacks({
        join: jest.fn(
          () =>
      ...
      store.set(PATH, { buildId: "rA", status: "running", aggregate: 1 });
      expect(cb.setBuilding).toHaveBeenCalledWith(true, "rA");
      ...
      // The dead runtime's join may still settle, but its invalidated
      // generation cannot emit a teardown into the replacement runtime.
      joinControl.resolve();
      await tick();
      expect(cb.setBuilding).not.toHaveBeenCalled();
    ...
    test("a local replacement invalidates errors and teardown from an old join", async () => {
      const joinControl = { reject: (_err: Error) => {} };
      const cb = makeCallbacks({
        join: jest.fn(
          () =>
            new Promise<void>((_resolve, reject) => {
              joinControl.reject = reject;
            }),
        ),
      });
      const coord = new BuildCoordinator(PROJECT_ID, PATH, cb);
      ...
      store.set(PATH, { buildId: "rA", status: "running", aggregate: 1 });
      // Emulate the editor's reset-only/local-stop state release, then start B.
      cb.setBuilding(false, "rA");
      coord.setLocalBuildId("local-B");
      coord.publishBuildStart("local-B", 2);
      (cb.setBuilding as jest.Mock).mockClear();

      joinControl.reject(new Error("late failure from A"));
      await tick();

      expect(cb.setError).not.toHaveBeenCalled();
      expect(cb.setBuilding).not.toHaveBeenCalled();
      expect(store.get(PATH)?.buildId).toBe("local-B");
      expect(store.get(PATH)?.status).toBe("running");
  ...
  describe("state machine", () => {
    ...
    test("running → running with new buildId triggers join (P2 fix)", async () => {
      ...
      expect(cb.join).toHaveBeenCalledTimes(2);
      expect(cb.join).toHaveBeenLastCalledWith("r2", 2, false, undefined);
