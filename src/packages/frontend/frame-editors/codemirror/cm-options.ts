...
export function cm_options(
  ...
  if (!theme || EDITOR_COLOR_SCHEMES[theme] == null) {
    console.warn(
      `codemirror theme '${theme}' not known -- fallback to '${DEFAULT_EDITOR_THEME}'`,
    );
  ...
  if (frame_tree_actions != null) {
    ...
    if (frame_tree_actions.sync != null) {
      extraKeys["Alt-Enter"] = (cm) =>
        frame_tree_actions.sync(frame_id, editor_actions, cm);
      extraKeys["Cmd-Enter"] = (cm) =>
        frame_tree_actions.sync(frame_id, editor_actions, cm);
    }
  ...
  if (opts.theme != null) {
    ...
    options.theme = DEFAULT_EDITOR_THEME;
  }
