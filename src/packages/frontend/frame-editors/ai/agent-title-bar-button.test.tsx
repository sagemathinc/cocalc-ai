...
describe("AgentTitleBarButton", () => {
  ...
  it("falls back to the current no-selector flow when there are no sessions", async () => {
    ...
    expect(
      screen.queryByLabelText("Recent agent sessions"),
    ...
    expect(languageModel).toHaveBeenCalledWith(
      "frame-1",
      expect.not.objectContaining({ agentSession: expect.anything() }),
      "selected text",
    );
  });
