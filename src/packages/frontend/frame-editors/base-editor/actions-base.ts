...
/*
Base Editor Actions -- common functionality shared by many frame editor actions.
...
WARNING: Some editors (e.g., for X11 and Jupyter) derive from this even though
they aren't used for code editing. This file intentionally avoids assumptions
about string-only docs (e.g., SyncString.to_str) in subclasses.
*/
...
> extends BaseActions<T | CodeEditorState> {
  ...
  protected getMergeCoordinator(): MergeCoordinator {
    if (!this.mergeCoordinator) {
      this.mergeCoordinator = new MergeCoordinator({
        getLocal: () => this._get_cm(undefined, true)?.getValue?.(),
        applyMerged: (merged) => this.applyMergedBuffer(merged),
      });
    }
  ...
  protected applyMergedBuffer(merged: string): void {
  ...
  protected handleRemoteSyncstringChange(remoteValue: string): void {
  ...
  protected _init_syncstring_value(): void {
    // no-op in base; string editors override this with to_str/merge logic
  }

  // Hook invoked after syncstring is ready (text editors may seed merge state).
  ...
  public focus_next_frame(): boolean {
    return this.focus_adjacent_frame(1, true);
  }
  ...
  public focus_next_frame_without_wrap(): boolean {
    return this.focus_adjacent_frame(1, false);
  }

  public focus_previous_frame_without_wrap(): boolean {
    return this.focus_adjacent_frame(-1, false);
  }

  private focus_adjacent_frame(direction: 1 | -1, wrap: boolean): boolean {
    ...
    if (currentIndex === -1) {
      ...
      const nextIndex = direction > 0 ? 0 : ids.length - 1;
      this.set_active_id(ids[nextIndex], true);
      return true;
    }
    const nextIndex = currentIndex + direction;
    if (!wrap && (nextIndex < 0 || nextIndex >= ids.length)) {
      return false;
    }
    const wrappedIndex = (nextIndex + ids.length) % ids.length;
    this.set_active_id(ids[wrappedIndex], true);
    return true;
  }
  ...
  public last_save_time(self_only: boolean = false): number {
    ...
    for (const path of files) {
      ...
      const t = (actions as BaseEditorActions).last_save_time(true);
      n = Math.max(n, t);
    }
    return n;
  }
  ...
  */
  ...
  languageModel = async (frameId: string, options: Options, input: string) => {
    await languageModelCreateChat({
      actions: this as BaseEditorActions<CodeEditorState>,
      frameId,
      options,
      input,
    });
