// forked from design-sync lib/bundle.mjs — minify:true + stub dead markdown/katex for <5MB upload
...
// design-sync fork: drop dead markdown/katex (~456KB) the wave-1 barrel never
// uses (MarkdownSection/PublicPage aren't exported) so the bundle fits 5MB.
const dsDeadStub = { name: 'ds-dead-stub', setup(b) {
  b.onLoad({ filter: /\/dist\/(markdown\/|misc\/math-to-html\.js)/ }, () => ({
...
function sharedBuildOptions({ nodePaths, tsconfig }) {
  ...
  const plugins = [reactShim, dsDeadStub];
