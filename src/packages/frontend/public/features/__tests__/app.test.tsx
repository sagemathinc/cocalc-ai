...
import { PUBLIC_COLORS, PUBLIC_DARK, PUBLIC_TYPE } from "../../theme";
...
> = {
  ...
  "dedicated-compute-page.tsx": ["#f4f9ff", "#fff8e8", "#ffffff"],
  "python-page.tsx": [
    "#278c83",
    "#2f6fda",
    "#389e0d",
...
describe("PublicFeaturesApp", () => {
  ...
  const auditedFeaturePages = [
    ...
    {
      marker: "Dedicated compute for heavier project workloads.",
      slug: "project-hosts",
    },
    {
  ...
  it("renders the features index", () => {
    ...
    expect(indexText.indexOf("Runtime")).toBeLessThan(
      indexText.indexOf("Documents"),
    );
    expect(indexText.indexOf("Documents")).toBeLessThan(
    ...
    const cliIcon = screen
      .getByRole("link", { name: /CoCalc CLI/i })
      .querySelector(".cocalc-feature-card-icon-cli") as HTMLElement;
    expect(cliIcon).toHaveStyle({ color: COLORS.ANTD_LINK_BLUE_DARK });
    expect(cliIcon).not.toHaveStyle({ color: COLORS.GRAY_D });
    expect(
      screen.getByText(
        "Run commands against CoCalc projects so scripts and agents can inspect context, run notebook checks, and leave outputs for review.",
      ),
    ).not.toBeNull();
    expect(
      screen.getByText(
        "Keep your notebooks, code, and history all together in one project.",
      ),
    ).not.toBeNull();
    expect(
      screen.getByText(
    ...
    expect(
      screen.queryByText("Collaborate Using Your Favorite Software"),
    ).toBeNull();
    expect(
      screen.queryByText("Collaborate While Using Your Favorite Software"),
    ).toBeNull();
    expect(
      screen.queryByText(
        "Agents, Collaborators, and Your Favorite Software",
      ),
    ).toBeNull();
    expect(
      screen.queryByText(
        "Your Agents, Collaborators, and Favorite Software — All Together.",
      ),
    ).toBeNull();
    expect(
      screen.queryByText(
        "Work with your favorite software, AI agents, and collaborators — all together.",
      ),
    ).toBeNull();
    expect(
    ...
    const linuxIcon = screen
      .getByRole("link", { name: /Linux Terminal/i })
      .querySelector(".cocalc-feature-card-icon-row > div") as HTMLElement;
    const automationsIcon = screen
      .getByRole("link", { name: /Project Automations/i })
      .querySelector(".cocalc-feature-card-icon-row > div") as HTMLElement;
    expect(linuxIcon).toHaveStyle({ color: COLORS.ANTD_LINK_BLUE_DARK });
    expect(linuxIcon.style.color).not.toBe(automationsIcon.style.color);
    expect(screen.queryByRole("link", { name: /^HTTP API$/ })).toBeNull();
    expect(
      screen
        .getByRole("link", { name: /Dedicated Compute/i })
        .getAttribute("href"),
    ).toBe("/features/project-hosts");
    const featureIndexLinks = Array.from(container.querySelectorAll("a")).map(
      (link) => link.getAttribute("href"),
    );
    expect(featureIndexLinks).toContain("/features/more-languages");
    ...
    expect(
      container.querySelectorAll(".cocalc-feature-link-list h3"),
    ).toHaveLength(6);
    const languageSummaries = Array.from(
      container.querySelectorAll(".cocalc-feature-list-summary"),
    );
    expect(languageSummaries).toHaveLength(6);
    for (const summary of languageSummaries) {
      expect(summary).toHaveStyle({ color: PUBLIC_COLORS.text });
    }
    expect(
      within(container.querySelector(".cocalc-feature-link-list")!).getByRole(
        "heading",
        { name: "Python", level: 3 },
      ),
    ).not.toBeNull();
    expect(container.textContent ?? "").not.toMatch(
      /same technical tools|workshop/i,
    );
    expect(
  ...
  it("keeps the feature index visually calm and route-specific", () => {
    ...
    expect(container.querySelectorAll(".cocalc-feature-group-row")).toHaveLength(
      4,
    );
    expect(container.querySelectorAll(".cocalc-feature-card-grid")).toHaveLength(
      3,
    );
    const groupLabelColumns = Array.from(
      container.querySelectorAll(".cocalc-feature-group-label-column"),
    );
    expect(groupLabelColumns).toHaveLength(4);
    for (const column of groupLabelColumns) {
      expect(column.classList.contains("ant-col-lg-5")).toBe(true);
      expect(column).toHaveStyle({
        alignItems: "center",
        display: "flex",
      });
    }
    for (const grid of container.querySelectorAll(".cocalc-feature-card-grid")) {
      expect(grid.closest(".ant-col-lg-19")).not.toBeNull();
    }
    expect(
      container.querySelector(".cocalc-feature-teaching-label-column"),
    ).toHaveClass("ant-col-lg-5");
    expect(
      container.querySelector(".cocalc-feature-teaching-card-column"),
    ).toHaveClass("ant-col-lg-19");
    const languageIcons = Array.from(
      container.querySelectorAll(".cocalc-feature-list-icon"),
    );
    expect(languageIcons).toHaveLength(6);
    for (const icon of languageIcons) {
      expect(icon).toHaveStyle({ alignSelf: "center" });
    }
    expect(
    ...
    expect(screen.queryByText("Notebooks and writing")).toBeNull();
    expect(
      screen.queryByRole("link", { name: "Project notes and Markdown" }),
    ).toBeNull();
  ...
  it("renders a detail page", () => {
    ...
    expect(
      screen.getByText(/edit Markdown, code, or notebooks/i),
    ).not.toBeNull();
    expect(
      screen.queryByText(/Use Codex to inspect files, edit Markdown or code/i),
    ).toBeNull();
    expect(
      screen.getByRole("img", {
        name: /CoCalc agent thread in a project/i,
      }),
    ).not.toBeNull();
    // Mock-UI labels (not headings): the agent panel says "Agent thread" /
    // "review thread", never the prior "Codex"/"chat history" labels.
    ...
    expect(screen.getByText("review thread")).not.toBeNull();
    ...
    expect(
      screen.queryByText(
        "Stay in a project when someone needs to pick up the work later.",
      ),
    ).toBeNull();
    ...
    expect(
      screen.queryByText(
        /Reviewers need the patch, checks, screenshots, and rationale/i,
      ),
    ).toBeNull();
    expect(container.querySelector(".feature-ai-context-panel")).toBeNull();
    expect(container.querySelector(".feature-ai-hero-row")).not.toBeNull();
    expect(container.querySelector(".feature-ai-thread-note")).not.toBeNull();
    expect(
      container.querySelectorAll(".feature-ai-related-link-label-separated"),
    ).toHaveLength(2);
  ...
  it("renders the richer jupyter feature page", () => {
    ...
    expect(
      container.querySelector(".cocalc-jupyter-evidence-row"),
    ).not.toBeNull();
    expect(
      container.querySelector(".cocalc-jupyter-balance-panel"),
    ).toHaveStyle({ height: "100%", minHeight: "240px" });
    expect(
    ...
    const pageText = container.textContent ?? "";
    expect(pageText).toContain("Kernel sessions stay inspectable");
    expect(pageText).toContain("Cell commands target real output");
    expect(pageText.match(/packages/g) ?? []).toHaveLength(1);
    expect(pageText).not.toMatch(
      /Kernels run beside project files|Data and packages stay nearby|instructors close/i,
    );
  ...
  it("renders the richer teaching feature page", () => {
    ...
    expect(
      screen.queryByText(
        /Students run assignments in shared projects that instructors can inspect/i,
      ),
    ).toBeNull();
    expect(
      screen.queryByText(
    ...
    expect(
      within(
        screen.getByText("Technical course workspace").closest("section")!,
      ).queryByRole("link", { name: "Compare operating models" }),
    ).toBeNull();
    expect(
      screen
  ...
  it("renders the richer terminal feature page", () => {
    ...
    const storyLayout = container.querySelector(
      ".cocalc-terminal-story-layout",
    );
    const storyCardColumn = container.querySelector(
      ".cocalc-terminal-story-card-column",
    );
    const storyCopyColumn = container.querySelector(
      ".cocalc-terminal-story-copy-column",
    );
    expect(storyLayout).not.toBeNull();
    expect(storyCardColumn).toHaveClass("ant-col-lg-9");
    expect(storyCopyColumn).toHaveClass("ant-col-lg-15");
    expect(
      (storyCardColumn?.compareDocumentPosition(storyCopyColumn as Node) ??
        0) &
        Node.DOCUMENT_POSITION_FOLLOWING,
    ).toBeTruthy();
    const terminalMock = screen.getByRole("img", {
      name: "Illustration of a CoCalc terminal beside project files",
    });
    const evidenceLayout = container.querySelector(
      ".cocalc-terminal-evidence-layout",
    );
    const evidenceColumn = container.querySelector(
      ".cocalc-terminal-evidence-column",
    );
    const contextColumn = container.querySelector(
      ".cocalc-terminal-context-column",
    );
    const contextGrid = container.querySelector(
      ".cocalc-terminal-context-grid",
    );
    expect(evidenceLayout).not.toBeNull();
    expect(evidenceColumn).toHaveClass("ant-col-lg-15");
    expect(contextColumn).toHaveClass("ant-col-lg-9");
    expect(contextGrid).not.toBeNull();
    expect(contextGrid).toHaveStyle(
      "grid-template-columns: repeat(2, minmax(0, 1fr))",
    );
    expect(
      contextGrid?.querySelectorAll(".cocalc-terminal-context-card"),
    ).toHaveLength(4);
    expect(
      (terminalMock.compareDocumentPosition(contextGrid as Node) &
        Node.DOCUMENT_POSITION_FOLLOWING) !==
        0,
    ).toBe(true);
    for (const label of [
      ".term files reopen in their folder",
      "One live stream for collaborators",
      "Sessions survive disconnects",
      "Codex can inspect project context",
    ]) {
      expect(screen.getByText(label)).not.toBeNull();
    }
    expect(screen.getByText(/side chat beside the session/i)).not.toBeNull();
    ...
    expect(
      screen.getByRole("link", { name: "Terminal field guide" }),
    ).toHaveAttribute(
      "href",
      "https://sagemathinc.github.io/cocalc-guides/terminal/",
    );
  });
  ...
  it.each([
    ...
    {
      name: "Illustration of a CoCalc Jupyter notebook beside project files",
      slug: "jupyter-notebook",
    },
    {
      name: "Illustration of a CoCalc course workspace connected to student projects",
      slug: "teaching",
    },
    {
      name: "Dedicated CoCalc compute capacity",
      slug: "project-hosts",
    },
    {
      name: "Illustration of Julia workflows in a CoCalc project",
      slug: "julia",
    },
    {
      name: "Illustration of R workflows in a CoCalc project",
      slug: "r-statistical-software",
    },
  ])("exposes the $slug mock panel as an image", ({ name, slug }) => {
    render(
      <PublicFeaturesApp
        config={{ help_email: "help@example.com", site_name: "Launchpad" }}
        initialRoute={{ slug, view: "detail" }}
  ...
  it("renders the richer linux environment page", () => {
    ...
    expect(container.querySelector(".cocalc-linux-final-band")).not.toBeNull();
    const reusableGrid = container.querySelector(
      ".cocalc-linux-reusable-grid",
    );
    expect(reusableGrid).toHaveStyle({
      display: "grid",
      gridTemplateColumns: "repeat(2, minmax(0, 1fr))",
    });
    expect(
      reusableGrid?.querySelectorAll(".cocalc-linux-reusable-grid-item"),
    ).toHaveLength(4);
    ...
    expect(screen.getByText(/python -m http\.server 8000/)).not.toBeNull();
    expect(
      screen.getByText(/curl --fail http:\/\/127\.0\.0\.1:8000\//),
    ).not.toBeNull();
    const linuxCodeBlock = screen.getByLabelText(
      "Linux package installation, service check, and verification commands",
    );
    expect(
      linuxCodeBlock.querySelector(".cocalc-code-block-scroll"),
    ).toHaveStyle({ paddingRight: "64px" });
    expect(
      within(linuxCodeBlock).getByRole("button", {
        name: "Copy to clipboard",
      }),
    ).toHaveStyle({ background: PUBLIC_COLORS.surface });
    expect(
      linuxCodeBlock.querySelector(".cocalc-code-block-fade"),
    ).not.toBeNull();
  ...
  it.each([
    ...
    expect(
      container.querySelector(".cocalc-feature-final-related-row"),
    ).not.toBeNull();
    expect(
      container.querySelector(".cocalc-feature-final-related-label"),
    ).toHaveStyle({ flex: "0 0 auto" });
    expect(
      container.querySelector(".cocalc-feature-final-related-links"),
    ).not.toBeNull();
    expect(screen.getByText(title)).not.toBeNull();
    expect(screen.getByText("Related")).not.toBeNull();
    expect(
  ...
  it("renders the compare feature page", () => {
    ...
    expect(
      screen.getByText(
        /Start with the shape of the work before pricing, procurement, or deployment takes over/i,
      ),
    ).not.toBeNull();
    expect(
      screen.getByText(
        /Best fit: ongoing work that needs shared execution, review, and handoff/i,
      ),
    ).not.toBeNull();
    expect(
      screen
    ...
    const supportHref =
      screen
        ...
        .getAttribute("href") ?? "";
    const supportParams = new URL(supportHref, "https://example.com")
      .searchParams;
    expect(supportHref).toEqual(expect.stringContaining("/support/new?"));
    expect(supportHref).toEqual(
      expect.stringContaining("context=feature-compare"),
    );
    expect(supportHref).toEqual(expect.stringContaining("type=purchase"));
    expect(supportHref).not.toContain("mailto:");
    expect(supportParams.get("title")).toBe(
      "Talk with CoCalc about workflow fit",
    );
