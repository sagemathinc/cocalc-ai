...
describe("BaseProject.start RootFS sealing", () => {
  ...
  it("restarts on a sealed managed RootFS when the current binding is unsealed", async () => {
    ...
    const project = new BaseProject("proj-1");
    project.computeQuota = jest.fn(async () => undefined);

    await project.start({ account_id: "acct-1", lro_op_id: "op-1" });

    expect(startProjectOnHostMock).toHaveBeenNthCalledWith(1, "proj-1", {
      account_id: "acct-1",
      lro_op_id: "op-1",
    });
    ...
    expect(stopProjectOnHostMock).toHaveBeenCalledWith("proj-1");
    expect(startProjectOnHostMock).toHaveBeenNthCalledWith(2, "proj-1", {
      account_id: "acct-1",
      lro_op_id: "op-1",
    });
  });
