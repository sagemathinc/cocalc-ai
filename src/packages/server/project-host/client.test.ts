...
describe("project-host client routing", () => {
  beforeEach(() => {
    ...
    bridgeHostControlMock = jest.fn(() => ({
      ...
      startRootfsBuild: jest.fn(async () => ({
        build_id: "b1",
        project_id: "p1",
        status: "running",
        created_at: new Date().toISOString(),
        paths: {
          dir: ".cocalc/rootfs-builds/b1",
          script: ".cocalc/rootfs-builds/b1/run.sh",
          log: ".cocalc/rootfs-builds/b1/build.log",
          status: ".cocalc/rootfs-builds/b1/status.json",
          events: ".cocalc/rootfs-builds/b1/events.ndjson",
        },
      })),
      getRootfsBuildStatus: jest.fn(async () => ({
        build_id: "b1",
        project_id: "p1",
        status: "succeeded",
        created_at: new Date().toISOString(),
      ...
      getRootfsBuildLog: jest.fn(async () => ({
        build_id: "b1",
        project_id: "p1",
        lines: 10,
        text: "",
        found: true,
        path: ".cocalc/rootfs-builds/b1/build.log",
      })),
      cancelRootfsBuild: jest.fn(async () => ({
        build_id: "b1",
        project_id: "p1",
        status: "canceling",
        signaled: true,
