...
export async function setProjectRunQuotaDiskMinimum({
  project_id,
  disk_quota_mb,
}: {
  project_id: string;
  ...
  const desired = Math.max(0, Math.ceil(disk_quota_mb));
  ...
  const current = numberFromUnknown(rows[0].run_quota?.disk_quota);
  const next = current == null ? desired : Math.max(current, desired);
...
export async function upsertLegacyMigrationProjectDiskQuotaOverride({
  project_id,
  legacy_project_id,
  disk_quota_mb,
  restored_used_bytes,
  ...
  const override = await setProjectEntitlementOverrideLocal({
    ...
    override: {
      enabled: true,
      project_defaults: {
        disk_quota: { mode: "minimum", value: desired },
      },
      metadata: {
        legacy_project_id,
        restored_used_bytes,
  ...
  await setProjectRunQuotaDiskMinimum({
    project_id,
    disk_quota_mb: desired,
  });
  return override;
