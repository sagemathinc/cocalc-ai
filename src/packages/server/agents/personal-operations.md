...
Personal messaging is available on every CoCalc site. The account preference
only controls whether the experimental UI is shown; it is not an authorization
boundary and does not pause or revoke existing connections. Names, rename
tombstones, grants, controls, and approval history are authoritative on the
execution principal's account home. Existing shared grants are not migrated or
used as fallback.
...
expired grants, controls, and terminal approval history. The error is explicit;
do not delete retained state to bypass it.
