...
export async function reservePersonalVmInTransaction(
  ...
  if (existing) return existing.pricing_snapshot.binding;
  const now = new Date();
  ...
  const binding: ComputeVmFundingBinding = {
    ...
    reservation_id: consentId,
    funding_epoch: consentId,
    resource_id: vm.id,
  ...
  const {
    ...
    VALUES ($1,$2,$3,$4,$5,$6,'compute-vm',$1,$1,$7,$8,$9,$10,$11,$12,$13,'reserved') RETURNING *`,
    [
      consentId,
      payer,
      hold.id,
...
export async function checkPersonalVmFunding(
  ...
  return withFundingAccountTransaction(opts.account_id, async (client) => {
    ...
    if (reservation.pricing_snapshot.request.provider === "gcp") {
      if (
