...
function fundingReview(
  ...
  return `${poolChange ? `<h2>${poolChange.terms.action === "close" ? "Close Pool And Revoke All Grants" : "Revise Pool And Student Limits"}</h2><p>Resulting limits and dates are shown below. Released funds return to the payer; reserved liabilities remain covered. Revoked grants cannot authorize new sponsored work. The pool must still be at version ${poolChange.terms.expected_version}.</p>` : ""}<dl><dt>Payer</dt><dd>${identity(review.payer)}</dd><dt>${poolChange ? "Resulting pool authorization (including spent and reserved funds)" : "Pool commitment"}</dt><dd>${usd(terms.amount_usd)}</dd>
    ...
    <h2>GCP Network Charges</h2><p>Public Internet traffic from a course-funded GCP VM costs USD 0.10/GB and is measured after traffic is sent. GCP samples this usage every 60 seconds, and a sample can take up to another 150 seconds to become visible. CoCalc checks for newly visible usage about every 60 seconds and requests a stop after the network spending threshold is observed. The payer accepts the actual finalized GCP egress charge, including potentially material usage reported after a student's allowance or the displayed pool commitment is exhausted. A mandatory provider-side run limit also stops each sponsored VM after at most 24 hours by default; restarting resets that limit. This exception applies only to measured GCP public egress; it does not increase student ceilings or authorize additional compute or storage.</p>
