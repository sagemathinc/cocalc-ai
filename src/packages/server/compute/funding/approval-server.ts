...
function fundingReview(
  ...
  if ("kind" in intent.terms && intent.terms.kind === "personalVMfallback") {
    ...
    return `<p>This authorizes charges to your personal account, not the course payer. The additional lifetime cap includes compute, egress and protected storage. Reaching the cap or end time stops personal compute; it does not authorize another payment source.</p>
      ...
      ${vm.home_volumes.length ? `<h2>Separately Authorized Home Volumes</h2><p>These disks use the same personal cap. Deleting the VM does not delete them. Their storage continues under its own funded deadline, no later than the dates below; exhausted or cancelled funding can end it earlier.</p>${vm.home_volumes.map((v) => `<section aria-label="${escapeHtml(v.name)}"><dl><dt>Volume</dt><dd>${escapeHtml(v.name)}</dd><dt>Volume ID</dt><dd>${escapeHtml(v.id)}</dd><dt>USD per hour</dt><dd>${escapeHtml(v.hourly_usd)}</dd><dt>Latest deletion (UTC)</dt><dd>${escapeHtml(v.storage_delete_at)}</dd></dl></section>`).join("")}` : "<p>No separate home volume is authorized by this consent.</p>"}`;
  }
