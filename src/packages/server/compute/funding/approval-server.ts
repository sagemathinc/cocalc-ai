...
function fundingReview(
  ...
  if ("kind" in intent.terms && intent.terms.kind === "personalVMfallback") {
    ...
    return `<p>This authorizes charges to your personal account, not the course payer. The additional lifetime cap includes compute, egress and protected storage. Reaching the cap or end time stops personal compute; it does not authorize another payment source.</p>
      ...
      ${vm.home_volumes.length ? `<h2>Separately Authorized Home Volumes</h2><table><thead><tr><th scope="col">Volume</th><th scope="col">Deletion deadline (UTC)</th></tr></thead><tbody>${vm.home_volumes.map((v) => `<tr><th scope="row">${escapeHtml(v.name)}<br>${escapeHtml(v.id)}</th><td>${escapeHtml(v.storage_delete_at)}</td></tr>`).join("")}</tbody></table>` : "<p>No separate home volume is authorized by this consent.</p>"}`;
  }
