...
export async function ensureCourseFundingApprovalSchema(): Promise<void> {
  // financial_approval_identities and financial_approval_sessions are owned by
  // db-schema/compute-funding and migrated by normal database schema sync before
  // services start. Only the remaining service-local tables are initialized here.
  await getPool()
