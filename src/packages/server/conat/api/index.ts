...
2. Run this script at the terminal:
    ...
    echo "require('@cocalc/server/conat/api').initAPI()" | COCALC_PRODUCT=launchpad COCALC_DEPLOYMENT_MODE=cloud DEBUG_CONSOLE=yes DEBUG=cocalc:* node


3. Optional: start more servers -- requests get randomly routed to exactly one of them:

    echo "require('@cocalc/server/conat').default()" | COCALC_PRODUCT=launchpad COCALC_DEPLOYMENT_MODE=cloud DEBUG_CONSOLE=yes DEBUG=cocalc:* node
    echo "require('@cocalc/server/conat').default()" | COCALC_PRODUCT=launchpad COCALC_DEPLOYMENT_MODE=cloud DEBUG_CONSOLE=yes DEBUG=cocalc:* node
