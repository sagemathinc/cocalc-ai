...
export async function init(envVars?: EnvVars) {
  ...
  const projectSecretsPath = process.env[PROJECT_SECRETS_ENV];
  if (projectSecretsPath) {
    sshEnvVars[PROJECT_SECRETS_ENV] = projectSecretsPath;
  }
  ...
  try {
    ...
    const envData = !isEmpty(sshEnvVars)
      ? Object.entries(sshEnvVars)
  ...
  const script = process.env.COCALC_PROJECT_SSH_START_SCRIPT?.trim();
  if (!script) {
    warn("COCALC_PROJECT_SSH_START_SCRIPT is not set; not starting sshd");
    return;
  }
