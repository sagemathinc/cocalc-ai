    ...
    if (
      ...
      try {
        const synced = await syncPublicViewerManifest({
          ...
          fallbackDescription:
            "Edit index.json to curate this viewer directory, or add index.html for a custom landing page.",
        });
...
export async function statusApp(id: string): Promise<AppStatus> {
  ...
  if (isUnmanagedServiceSpec(serviceSpec)) {
    ...
    return {
      ...
      warnings: [
        ready
          ? "Process lifecycle is unmanaged. CoCalc can open this app, but it does not start or stop the server."
