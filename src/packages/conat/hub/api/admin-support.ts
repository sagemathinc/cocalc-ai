...
export const ADMIN_SUPPORT_CONVENTIONS = {
  ...
  vocabulary: [
    ...
    "Where the interface uses more than one name for one thing, use the name shown where the user is looking and do not present any one of them as the only correct name. Current cases: the Agents rail tab and the Codex assistant; the Image rail tab and the RootFS Images admin page; the Docs rail tab, whose page and side panel both show the title Help for this project; the VMs rail tab, whose page and side panel are titled Virtual machines; the Log rail tab, whose page is titled Recent Files or Project Activity Log to match its Files and Activity toggle, and whose side panel is titled Recent; and project settings, which the Settings rail tab opens as a page titled Project Settings or a side panel titled Status and Settings, and which Quick Navigation opens as a side panel titled Users.",
