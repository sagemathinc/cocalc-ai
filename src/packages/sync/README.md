...
Run these commands from this package after the workspace dependencies have
been prepared. Jest transforms TypeScript test sources through `ts-jest`; it
does not run this package's tests exclusively from `dist/`. Some workspace
imports, including the configured util aliases, use compiled `dist` output,
so keep the relevant dependency builds current.
