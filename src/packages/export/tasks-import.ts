...
export async function importTaskBundle(
  ...
  if (!rawTargetPath) {
    throw new Error("unable to determine import target; pass --target explicitly");
  }
  const targetPath = resolve(rawTargetPath);

  const baseRows = parseJsonlRows(await source.readText("document.jsonl"), "document.jsonl")
    .filter(isTaskRow)
    .map(normalizeTaskRow);
  const desiredRows = parseJsonlRows(await source.readText("tasks.jsonl"), "tasks.jsonl")
    .filter(isExportTaskRow)
    .map(normalizeExportTaskRow);

  rejectLocalAssetRefs(baseRows.map((row) => row.desc), "document.jsonl");
  rejectLocalAssetRefs(desiredRows.map((row) => row.content), "tasks.jsonl");

  let currentRows: TaskRow[] = [];
  try {
    currentRows = (await readJsonlRows(targetPath)).filter(isTaskRow).map(normalizeTaskRow);
  } catch (err) {
  ...
  const preserved = Array.from(currentMap.keys()).filter((taskId) => !touched.has(taskId)).length;
...
function normalizeTaskRow(row: TaskRow): TaskRow {
  return {
    ...
    desc: `${row.desc ?? ""}`,
    position: typeof row.position === "number" && Number.isFinite(row.position)
      ? row.position
      : undefined,
    last_edited:
...
function conflictingFields(base: TaskRow, current: TaskRow, desired: TaskRow): string[] {
  const fields: string[] = [];
  for (const field of MANAGED_FIELDS) {
    ...
    if (currentChanged && desiredChanged && !fieldValueEqual(currentValue, desiredValue)) {
      fields.push(field);
    }
  }
  return fields;
...
function taskSnapshotEqual(a: TaskRow, b: TaskRow): boolean {
  return MANAGED_FIELDS.every((field) => fieldValueEqual(snapshotField(a, field), snapshotField(b, field)));
}

function mergeTaskRows(base: TaskRow, current: TaskRow, desired: TaskRow): TaskRow {
...
function rejectLocalAssetRefs(values: Array<string | undefined>, fileName: string): void {
...
function serializeTaskRow(row: TaskRow): TaskRow {
  return {
    ...
    ...(typeof row.last_edited === "number" ? { last_edited: row.last_edited } : {}),
