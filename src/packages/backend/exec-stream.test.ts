...
describe("executeStream function - unit tests", () => {
  ...
  it("drops live chunks fully covered by the asyncCache snapshot", async () => {
    // Deterministic overlap regression test: the snapshot already contains
    // "first\n"; the next batch flush re-emits those same bytes (offset 0)
    // because the cache is updated immediately while the emitter flushes
    // from a ≤100ms buffer. The overlap must be discarded.
    mockExecuteCode.mockImplementation(async (_options) => ({
      type: "async",
      job_id: "test-job-id",
      pid: 1234,
    ...
    // Buffer flush re-sends the snapshotted bytes with their offset, then
    // fresh data past the snapshot arrives.
    mockUpdates.emit(mockEventKey("stdout", "test-job-id"), "first\n", 0);
    mockUpdates.emit(mockEventKey("stdout", "test-job-id"), "second\n", 6);
    mockUpdates.emit(mockEventKey("finished", "test-job-id"), {
      type: "async",
    ...
    const calls = userCallback.mock.calls.map((c) => c[0]);
    const jobEvent = calls.find((e) => e?.type === "job");
    expect(jobEvent.data.stdout).toBe("first\n");
    const stdoutEvents = calls.filter((e) => e?.type === "stdout");
    // The fully-overlapped "first\n" chunk must NOT be forwarded.
    expect(stdoutEvents).toEqual([{ type: "stdout", data: "second\n" }]);
    expect(jobEvent.data.stdout + "second\n").toBe("first\nsecond\n");
  });

  it("slices partially overlapping live chunks at the snapshot boundary", async () => {
    // Snapshot caught the cache mid-chunk: "first\nse" (8 chars). The
    // flush then emits "second\n" at offset 6 — the first two chars are
    // already in the snapshot and must be sliced off.
    mockExecuteCode.mockImplementation(async (_options) => ({
      type: "async",
      job_id: "test-job-id",
      pid: 1234,
...
describe("updates EventEmitter streaming", () => {
  ...
  it("late joiner does not receive duplicated output (snapshot/live overlap)", async () => {
    // Regression test: the asyncCache is updated immediately on every data
    // event, while `updates` emits from a ≤100ms batch buffer. A late
    // joiner used to snapshot bytes from the cache and then receive some
    // of those same bytes again when the buffer flushed, producing e.g.
    // "first\nfirst\nsecond\n". The offset-based reconciliation must make
    // snapshot + live chunks reconstruct the exact output.
    const { executeCode } = await import("./execute-code");
    const { executeStream } = await import("./exec-stream");

    const { asyncCache } = await import("./execute-code");
    const aggregate = Date.now(); // shared aggregate → joiner attaches to same job
    const command = "sleep 0.15; echo 'first'; sleep 1; echo 'second'";

    // Originator starts the job.
    const job = await executeCode({
      command,
      bash: true,
      async_call: true,
    ...
    // Attach as soon as "first" lands in the asyncCache — i.e. inside the
    // window where the cache is already updated but the ≤100ms batch
    // flush has NOT yet emitted those bytes. Attaching here means the
    // joiner's snapshot overlaps the upcoming flush, exercising the
    // offset-based reconciliation. (The deterministic unit tests above
    // pin the overlap semantics exactly; this one exercises the real
    // wiring end-to-end.)
    while (!asyncCache.get(job.job_id)?.stdout?.includes("first")) {
      await delay(2);
    }

    // Late joiner attaches via the same command+aggregate.
    const streamEvents: any[] = [];
    await new Promise<void>((resolve) => {
      void executeStream({
        command,
    ...
    const jobEvent = streamEvents.find((e) => e.type === "job");
    expect(jobEvent).toBeDefined();
    const liveStdout = streamEvents
      .filter((e) => e.type === "stdout")
      .map((e) => e.data)
      .join("");
    const combined = (jobEvent.data.stdout ?? "") + liveStdout;
    expect(combined).toBe("first\nsecond\n");

    // The final result must also be exact (no duplication there either).
    const doneEvent = streamEvents.find((e) => e.type === "done");
