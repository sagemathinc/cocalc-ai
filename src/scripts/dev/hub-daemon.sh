...
load_config() {
  ...
  COCALC_SETTING_PROJECT_HOSTS_ROUTE_MODE="${COCALC_SETTING_PROJECT_HOSTS_ROUTE_MODE:-public}"
  ...
  COCALC_CONAT_AUTH_FAIL_LIMIT="${COCALC_CONAT_AUTH_FAIL_LIMIT:-1000}"
  COCALC_CONAT_AUTH_FAIL_BLOCK_MS="${COCALC_CONAT_AUTH_FAIL_BLOCK_MS:-15000}"
...
software_endpoint_mode() {
  if [ -n "$HUB_SOFTWARE_ENDPOINT_MODE" ]; then
    echo "$HUB_SOFTWARE_ENDPOINT_MODE"
  elif [ "$HUB_USE_LOCAL_SOFTWARE" = "1" ]; then
    echo "local"
  else
    echo "remote"
  fi
...
start_cluster_bay() {
  ...
  (
    ...
    export COCALC_CONAT_AUTH_FAIL_LIMIT
    export COCALC_CONAT_AUTH_FAIL_BLOCK_MS
