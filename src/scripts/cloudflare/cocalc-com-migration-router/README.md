...
`cocalc-com-migration-router`, intended for the `cocalc.com/*` route. Check the
live Worker configuration separately before treating that attachment as current.
...
The removed path roots `/github`, `/gist`, and `/rajacuan` instead return HTTP
410; those paths are not sent to the legacy resolver.
...
The following deployment configuration is a recorded operator reference, not
a live readback. It uses the Cloudflare Workers Scripts API with account
